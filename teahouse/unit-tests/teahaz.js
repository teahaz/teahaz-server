...
class Chatroom
{
    constructor({server, username, password, ...args})
    {
        ...
        /*
         * Although chatroom_id is crusial for the server running,
         * it is not needed for create chatroom, as there the server
         * assings a new chatroom_id.
         *
         * It is however needed for every other function.
         */
        this.chatroom_id = args.chatroom_id;
         ...
         *
         * This needs to be paired up with the chatroom_id
         ...
         * format = [
         *      {
         *        channel_id: '9541e2de-ecc9-11eb-b7fd-b42e99435986',
         *        name: 'default',
         *        permissions: [
         *            {
         *               class_id: '1',
         ...
         *
         * format = [
         *      { class_id: '0', name: 'constructor' },
         *      { class_id: '1', name: 'default' }
         *    ],
         */
        this.classes = [];
    ...
    {
        this.chatroom_name = response.data['chatroom-name'];
        this.chatroom_id   = response.data['chatroom-id'];
    ...
    async enable({type, ...args}={})
    {
         ...
         *      if chatroom_id is not set: create_chat
         *      if invite_id is set: use invite
         *      else: login
         */
        ...
        if (type == undefined)
        {
            ...
            else if (args.invite_id || this.invite_id)
                type = 2;
    ...
    async login({callback_success, callback_error}={})
    {
        ...
        assert(this.chatroom_id , "'chatroom_id' variable has not been set.");


        return axios({
            method: 'post',
            url: `${this.server}/api/v0/login/${this.chatroom_id}`,
            header: {
                "User-Agent": USER_AGENT,
                "Content-Type": "application/json"
            },
    ...
    async use_invite({invite_id, username, nickname, password, callback_success, callback_error}={})
    {
        /*
        ...
        assert((invite_id || this.invite_id), "'invite-id' variable has not been set!");
        ...
        return axios({
            method: 'post',
            url: `${this.server}/api/v0/invites/${this.chatroom_id}`,
            header: {
                "User-Agent": USER_AGENT,
                "Content-Type": "application/json"
            },
            data: {
                "invite-id": invite_id || this.invite_id,
    ...
    async create_invite({uses, classes, expiration_time: expiration_time, callback_success, callback_error}={})
    {
        ...
        assert((classes == undefined || Array.isArray(classes)), "'classes' (optional) variable has to be an array of class _ids");
        ...
        let headers = {
            "User-Agent": USER_AGENT,
            "Content-Type": "application/json",
            "Cookie": `${this.chatroom_id}=${this.cookie}`,
    ...
    async get_chat_info({callback_success, callback_error}={})
    {
        ...
        return axios({
            method: 'get',
            url: `${this.server}/api/v0/chatroom/${this.chatroom_id}`,
            headers:
            {
                "User-Agent": USER_AGENT,
                "Content-Type": "application/json",
    ...
    async create_channel({channel_name, permissions, callback_success, callback_error}={})
    {
        ...
        assert(this.username, "'chatroom_id' variable is undefined.");
        assert(this.chatroom_id, "'chatroom_id' variable is undefined.");
        ...
        return axios({
            method: 'post',
            url: `${this.server}/api/v0/channels/${this.chatroom_id}`,
            headers:
            {
                "User-Agent": USER_AGENT,
                "Content-Type": "application/json",
    ...
    async send_message({message, channel_id, reply_id, callback_success, callback_error}={})
    {
        assert(typeof(message) == 'string', "'message' has to be of type string.")
        assert(channel_id, "'channel-id' variable has not been set!")
        ...
        return axios({
            method: 'post',
            url: `${this.server}/api/v0/messages/${this.chatroom_id}`,
            headers:
            {
                "User-Agent": USER_AGENT,
                "Content-Type": "application/json",
    ...
    async get({time, channel_id, callback_success, callback_error}={})
    {
        ...
        // so the channel_id will only be set if its defined.
        //
